...
tau_t = .5;
n = 80;
act = (1-exp(-t/tau_t))*gp(n) - (1-exp(-max(0,t-1)/tau_t))*d(n);
subplot(122)
plot(t,act,'linewidth',3)
set(gca,'fontsize',14)
xlabel('Time')
ylabel('Activity')
title('Negative top-down feedback')
set(gca,'YTickLabel',[])
set(gca,'XTickLabel',[])
xlim([0 5])
ylim([0 0.015])
...
xlabel('Time')
ylabel('Activity')
title('Positive top-down feedback')
set(gca,'YTickLabel',[])
set(gca,'XTickLabel',[])
xlim([0 5])
ylim([0 0.015])

%% Linear Decoding ability over time

dec = (1-exp(-t/tau_t))*(gp'*d) - .7*(1-exp(-max(0,t-1)/tau_t))*(d'*d);


plot(t,dec,'linewidth',2)
hold on
